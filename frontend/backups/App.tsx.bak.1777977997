 ...
 * Активные страницы: План/Факт (бывшая Маржа), Реклама, Настройки.
 *
 * Сверху всегда виден баннер о необходимости обновить ИЛ/ИРП,
 * если они старше 7 дней (читает /settings/ при загрузке).
 */
import { useEffect, useState } from "react";
import { ConfigProvider, Layout, Menu, Alert, Button } from "antd";
import {
  LineChartOutlined, NotificationOutlined, SettingOutlined,
...
type PageKey = "margin" | "advertising" | "settings";

// Сколько прошло дней с ISO-даты
...
// Баннер: показывается, если ИЛ или ИРП старше 7 дней.
...
const PAGES: Record<PageKey, { label: string; node: React.ReactNode }> = {
  margin:      { label: "План/Факт", node: <Margin /> },
      ...
      <Layout style={{ minHeight: "100vh" }}>
        ...
        >
          ...
          <Menu
            ...
            items={[
              { key: "margin",      icon: <LineChartOutlined />,    label: "План/Факт" },
