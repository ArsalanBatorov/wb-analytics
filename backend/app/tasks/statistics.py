...
async def _daily_sync():
    ...
    today = date.today()
    d6_ago = today - timedelta(days=6)

    print(f"[DAILY_SYNC] Starting full sync {d6_ago}..{today}")
    ...
    try:
        print("[DAILY_SYNC] 2/5 Daily stats (6 days)...")
        await sync_daily_stats(days=6)
    except Exception as e:
        print(f"[DAILY_SYNC] Stats error: {e}")

    try:
        print("[DAILY_SYNC] 3/5 Ad stats (6 days)...")
        await sync_ad_stats(days=6)
    except Exception as e:
        print(f"[DAILY_SYNC] Ads error: {e}")

    try:
        print("[DAILY_SYNC] 4/5 Realization...")
        await sync_realization_daily(d6_ago, today)
    except Exception as e:
